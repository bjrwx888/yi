...
namespace Yi.Framework.WeChatPay.Helper
{
    public class PayApi
    {
        ...
        */
        public PayData UnifiedOrder(PayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", _IPayConfig.GetAppID());//公众账号ID
            inputObj.SetValue("mch_id", _IPayConfig.GetMchID());//商户号
            inputObj.SetValue("spbill_create_ip", _IPayConfig.GetIp());//终端ip	  	    
            ...
            // 发起http请求
            string response = _PayHttpService.Post(xml, url, false, timeOut);
        ...
        */
        public PayData OrderQuery(PayData inputObj, HttpContext httpContext, int timeOut = 6)
        {
            ...
            string response = _PayHttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
