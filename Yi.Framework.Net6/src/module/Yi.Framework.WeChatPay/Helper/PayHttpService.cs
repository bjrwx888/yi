...
#pragma warning restore SYSLIB0014 // 类型或成员已过时
                request.UserAgent = string.Format("WXPaySDK/{3} ({0}) .net/{1} {2}", Environment.OSVersion, Environment.Version, _IPayConfig.GetMchID(), typeof(PayHttpService).Assembly.GetName().Version);
                request.Method = "POST";
                ...
                if (isUseCert)//是否使用证书--没用证书
                {
                    ...
                    X509Certificate2 cert = new X509Certificate2(path + _IPayConfig.GetSSlCertPath(), _IPayConfig.GetSSlCertPassword());
